...
// Bottom banner: running token saved counter with reset
// ============================================================
...
export function CompressTab({ settings, onCompressed }: Props) {
  ...
  const [totals, setTotals]         = useState<TokenTotals>(() => getTokenTotals());
  const [sessionFlash, setFlash]    = useState(false);

  // Refresh totals whenever a new result arrives and record the saving
  useEffect(() => {
    if (!result) return;
    const saved = result.originalTokens - result.compressedTokens;
    recordCompression(saved);
    setTotals(getTokenTotals());

    // Flash animation on the session counter
    if (saved > 0) {
      setFlash(true);
  ...
  function handleResetAlltime() {
    if (window.confirm('Reset all-time token counter? This cannot be undone.')) {
      resetAlltime();
      setTotals(getTokenTotals());
    }
  }
  ...
  return (
    <div className="compress-page">
      ...
      {/* ---- Token saved banner ---- */}
      <div className="token-counter-banner">
        <div className="token-counter-stats">

          <div className="token-stat">
            <span className={`token-stat-value session ${sessionFlash ? 'flash' : ''}`}>
              {totals.sessionSaved.toLocaleString()}
            </span>
            <span className="token-stat-label">tokens saved this session</span>
            {totals.sessionCompressions > 0 && (
              <span className="token-stat-sub">
                {totals.sessionCompressions} compression{totals.sessionCompressions !== 1 ? 's' : ''}
              </span>
            )}
          </div>
          ...
          <div className="token-stat">
            <span className="token-stat-value alltime">
              {totals.alltimeSaved.toLocaleString()}
            </span>
            <span className="token-stat-label">tokens saved all time</span>
            {totals.alltimeCompressions > 0 && (
              <span className="token-stat-sub">
                {totals.alltimeCompressions} total compression{totals.alltimeCompressions !== 1 ? 's' : ''}
              </span>
            )}
          </div>
        ...
        <div className="token-counter-actions">
          <button
            className="btn btn-secondary btn-sm"
            onClick={handleNewChat}
            title="Reset session counter and clear input — use when starting a new chat"
          >
            ↺ New Chat
          </button>
          <button
      ...
      <div className="compress-layout">
        ...
        <div className="compress-input-panel">
          ...
          <textarea
            ...
            placeholder={"Paste your prompt here…\n\nTry: \"Hello! I was just wondering if you could basically help me understand how transformers work, if that's okay?\""}
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: 4 }}>
                  {result.originalTokens > result.compressedTokens
                    ? `Saved ~${(result.originalTokens - result.compressedTokens).toLocaleString()} tokens this compression`
